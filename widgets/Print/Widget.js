...
define([
  ...
  return declare([BaseWidget], {
    ...
    _initPrinter: function () {
      this._getPrintTaskURL(this.appConfig.portalUrl).then(
        lang.hitch(this, function (printServiceUrl) {
          this.shelter.show();
          var asyncDef = this.isAsync(printServiceUrl);
          asyncDef
            .then(
              lang.hitch(this, function (async) {
                ...
                // this.print.templateInfos.push({
                //   layoutTemplate: "Tabloid ANSI B Portrait",
                //   pageSize: [11, 17],
                //   pageUnits: "INCH",
                //   webMapFrameSize: [10, 13.61],
                //   layoutOptions: {
                //     hasTitleText: true,
                //     hasAuthorText: true,
    ...
    _getPrintTaskURL: function (portalUrl) {
      ...
      def.then(
        lang.hitch(this, function (response) {
          var printServiceUrl =
            response &&
            response.helperServices &&
            response.helperServices.printTask &&
            response.helperServices.printTask.url;
          if (printServiceUrl) {
    ...
    isAsync: function (serviceURL) {
      ...
      var portalNewPrintUrl = portalUrlUtils.getNewPrintUrl(
        this.appConfig.portalUrl
      );

      if (
        serviceUrl === portalNewPrintUrl ||
