          ...
          drawGRG.isNameAvailable(checkServiceNameUrl, token,
            ...
            if (response0.available) {
              ...
              array.forEach(this._layerList, lang.hitch(this, function (layer) {
                if (layer._json) {
                  if (dojoJSON.parse(layer._json).name) {
                    if (dojoJSON.parse(layer._json).name === layerName) {
                      selectedLayer = layer;
                    }
                  }
                }
              ...
              if (selectedLayer) {
                //Push features to new layer
                var newFeatureLayer =
                  new FeatureLayer(selectedLayer.url + "?token=" + token, {
                    id: layerName,
                    outFields: ["*"]
                  });
                newFeatureLayer._wabProperties = {
                ...
                var newURL = '<br /><a role="link" tabindex="0" aria-label="' +
                  this.nls.successfullyPublished + '" href="' + this.appConfig.portalUrl +
                  "home/item.html?id=" + dojoJSON.parse(selectedLayer._json).serviceItemId + '" target="_blank">';
                this.publishMessage.innerHTML =
                  this.nls.successfullyPublished.format(newURL) + '</a>';
